...
exports.IFoodMerchantStatusService = void 0;
const axios_1 = __importDefault(require("axios"));
const ifoodTokenService_js_1 = require("../../../services/ifood-token-service/src/ifoodTokenService.js");
const schedule = __importStar(require("node-schedule"));
// Get supabase client instance
const supabase = (0, ifoodTokenService_js_1.getSupabaseClient)();
class IFoodMerchantStatusService {
     ...
     */
    static async fetchMerchantStatus(merchantId, accessToken) {
        try {
            const response = await axios_1.default.get(this.IFOOD_STATUS_URL.replace('{merchantId}', merchantId), {
                headers: {
     ...
     */
    static async fetchOpeningHours(merchantId, accessToken) {
        try {
     ...
     */
    static addMinutesToTime(startTime, durationMinutes) {
     ...
     */
    static calculateIfOpen(openingHours) {
        const now = new Date();
     ...
     */
    static async updateMerchantStatus(merchantId, isOpen) {
        try {
     ...
     */
    static async saveOpeningHoursToDatabase(merchantId, shifts) {
        try {
     ...
     */
    static async updateOpeningHours(merchantId, dayOfWeek, // "MONDAY", "TUESDAY", etc
    startTime, // "08:00:00"
    endTime, // "18:00:00"
    accessToken) {
        try {
            ...
            const response = await axios_1.default.put(this.IFOOD_HOURS_URL.replace('{merchantId}', merchantId), putBody, {
                headers: {
                    'accept': 'application/json',
     ...
     */
    static async createScheduledPause(merchantId, startDate, // ISO format: "2025-01-17T14:00:00Z" - OBRIGATÓRIO
    endDate, // ISO format: "2025-01-17T18:00:00Z" - OBRIGATÓRIO
    description, // Descrição - OBRIGATÓRIO 
    accessToken, // OBRIGATÓRIO
    userId, // OBRIGATÓRIO - para salvar na tabela local
    reason // Motivo da pausa - Opcional
        ...
        try {
            ...
            const response = await axios_1.default.post(this.IFOOD_INTERRUPTIONS_URL.replace('{merchantId}', merchantId), requestBody, {
                headers: {
                    'accept': 'application/json',
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${accessToken}`
     ...
     */
    static async listScheduledPauses(merchantId) {
        try {
     ...
     */
    static async removeScheduledPause(merchantId, interruptionId, accessToken) {
        try {
            console.log(`🗑️ Removing scheduled pause: ${interruptionId} for merchant: ${merchantId}`);
            // First, try to remove from iFood API
            try {
                const response = await axios_1.default.delete(`${this.IFOOD_INTERRUPTIONS_URL.replace('{merchantId}', merchantId)}/${interruptionId}`, {
                    headers: {
                        'accept': 'application/json',
                        'Authorization': `Bearer ${accessToken}`
                    }
     ...
     */
    static async deleteOpeningHours(merchantId, dayOfWeek, // "MONDAY", "TUESDAY", etc
    accessToken) {
        try {
     ...
     */
    static async fetchInterruptionsFromiFood(merchantId, accessToken) {
        try {
            console.log(`🔄 Fetching interruptions from iFood API for merchant: ${merchantId}`);
            const response = await axios_1.default.get(this.IFOOD_INTERRUPTIONS_URL.replace('{merchantId}', merchantId), {
                headers: {
                    'accept': 'application/json',
                    'Authorization': `Bearer ${accessToken}`
                }
     ...
     */
    static async syncInterruptionsWithiFood(merchantId, accessToken) {
        try {
            ...
            const ifoodIds = new Set(ifoodInterruptions.map((item) => item.id));
     ...
     */
    static async checkSingleMerchantStatus(merchantId) {
        try {
            // Get merchant data
            ...
            // Get token
            const tokenData = await (0, ifoodTokenService_js_1.getTokenForUser)(merchant.user_id);
            if (!tokenData || !tokenData.access_token) {
                console.error('No access token found');
                return null;
            }
            // Fetch opening hours
            const { success, hours } = await this.fetchOpeningHours(merchantId, tokenData.access_token);
            ...
            const { success: statusSuccess, data: ifoodStatus } = await this.fetchMerchantStatus(merchantId, tokenData.access_token);
     ...
     */
    static async checkAllMerchantStatuses() {
        try {
            console.log('\n🚀 ================== POLLING INICIADO ==================');
            ...
            for (let i = 0; i < merchants.length; i += batchSize) {
                ...
                await Promise.all(batch.map(async (merchant) => {
                    try {
                        ...
                        const tokenData = await (0, ifoodTokenService_js_1.getTokenForUser)(userId);
     ...
     */
    static startScheduler(intervalMinutes = 1) {
...
exports.IFoodMerchantStatusService = IFoodMerchantStatusService;
IFoodMerchantStatusService.IFOOD_STATUS_URL = 'https://merchant-api.ifood.com.br/merchant/v1.0/merchants/{merchantId}/status';
IFoodMerchantStatusService.IFOOD_HOURS_URL = 'https://merchant-api.ifood.com.br/merchant/v1.0/merchants/{merchantId}/opening-hours';
IFoodMerchantStatusService.IFOOD_INTERRUPTIONS_URL = 'https://merchant-api.ifood.com.br/merchant/v1.0/merchants/{merchantId}/interruptions';
// Day mapping
IFoodMerchantStatusService.DAY_MAP = {
    'MONDAY': 1,
    'TUESDAY': 2,
    'WEDNESDAY': 3,
    'THURSDAY': 4,
...
exports.default = IFoodMerchantStatusService;
